...
so a listing never mixes a newer file with an older loaded assembly. Framework bodies differ between
runtime versions, so expect the shape to match and the details to move.
