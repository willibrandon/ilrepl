...
public static class MemberTokenFixture
{
    ...
    public static byte[] Create(string kind, string dispatch)
    {
            ...
            bool constructor = false)
        {
            ...
            new BlobEncoder(signature).MethodSignature(genericParameterCount: generic ? 1 : 0,
                ...
                returns =>
                {
                    ...
                    {
                        EncodeType(returns.Type(), result);
                    }
                }, arguments =>
                {
                    foreach (var parameter in parameters)
        ...
        for (var index = 0; index < 8; index++)
        {
            Members(MetadataTokens.TypeDefinitionHandle(index + 2));
        }
        ...
        void LoadReceiver()
        {
            ...
            if (kind is "Array" or "ByRef" or "Constructed")
            {
                ...
                if (kind == "Array")
                {
                    encoded.SZArray().Type(owner, false);
                }
        ...
        else if (route == "lookalike")
        {
            instructions.Call(MetadataTokens.MethodDefinitionHandle(39));
        }
        else
        {
            if (route == "invoke")
            ...
            else if (route is "delegate" or "named" or "pointer")
            {
                ...
                if (route != "pointer")
                {
                    LoadType(SignatureType(typeof(Func<int>)));
                }
        ...
        {
            Method("get_MetadataToken", typeof(int), [], Constant());
        }
