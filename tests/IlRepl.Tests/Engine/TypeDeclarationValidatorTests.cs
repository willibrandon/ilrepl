...
public sealed class TypeDeclarationValidatorTests
{
    ...
    public void Interfaces_MustBeImplemented()
    {
        ...
        Assert.Contains("class Circle must implement instance float64 IArea::Area() (declare a public virtual method",
            CloseRefused(missing));
        var notVirtual = Load([.. IArea, ".class public Circle implements IArea {",
            ".method public instance float64 Area() { ldc.r8 1; ret }"]);
        Assert.Contains("Area matches but is not virtual; add virtual to its header", CloseRefused(notVirtual));
        var implicitly = Load([.. IArea, ".class public Circle implements IArea {",
            ".method public virtual instance float64 Area() { ldc.r8 1; ret }"]);
        Assert.AreEqual("end of class Circle", implicitly.AddLine("}").Message);
        var explicitly = Load([.. IArea, ".class public Circle implements IArea {",
            ".method private virtual instance float64 Size() { .override IArea::Area; ldc.r8 1; ret }"]);
        ...
        var defaulted = Load(".class interface public abstract IArea {", ".method public virtual instance float64 Area() { ldc.r8 1; ret }",
            "}", ".class public Circle implements IArea {");
    ...
    {
        var session = Load(".class interface public abstract IZero {", ".method public static abstract virtual int32 Zero() { }", "}",
            ".class public Num implements IZero {");
        Assert.Contains("must implement static int32 IZero::Zero() (declare a static method with that name and signature)",
            CloseRefused(session));
    ...
    public void AbstractMembers_MustBeImplementedUnlessAbstract()
    {
        ...
        Assert.Contains(
            "class Tri must implement abstract instance int32 Shape::Sides() (declare a virtual method with that name and signature, or " +
            "mark the class abstract)", CloseRefused(session));
        ...
        var newslot = Load([.. AbstractShape, ".class public Tri extends Shape {",
            ".method public virtual newslot instance int32 Sides() { ldc.i4 3; ret }"]);
    ...
    {
        var session = Load([.. IArea, ".class public Circle {",
            ".method private virtual instance float64 Size() { .override IArea::Area; ldc.r8 1; ret }"]);
        Assert.Contains("class Circle cannot .override IArea::Area: IArea is not one of its bases or interfaces (add implements IArea)",
            CloseRefused(session));
    }
    ...
    {
        Assert.Contains("cannot extend sealed type Final",
            Assert.ThrowsExactly<ReplException>(() => Load(".class public sealed Final { }", ".class public More extends Final {"))
            .Message);
        Assert.Contains("cannot extend sealed type Color",
            Assert.ThrowsExactly<ReplException>(() => Load(".class public Color extends [System.Runtime]System.Enum {",
            ".field public specialname rtspecialname int32 value__", "}", ".class public Shade extends Color {")).Message);
        var nested = Load(".class public Outer {", ".class nested public sealed Final { }",
            ".class nested public More extends Outer/Final { }");
    ...
    {
        var session = Load(".class public explicit Union extends [System.Runtime]System.ValueType {", ".field [0] public int32 I",
        ...
        var fine = Load(".class public explicit Union extends [System.Runtime]System.ValueType {", ".field [0] public int32 I",
            ".field [0] public float32 F", ".field public static int32 Count");
    ...
    {
        Assert.Contains("cannot be both class and valuetype",
            Assert.ThrowsExactly<ReplException>(() => Load(".class public Box`1<class valuetype T> {")).Message);
    ...
    public void GenericInterface_MatchedAfterSubstitution()
    {
        ...
        var closed = Load([.. IBox, ".class public IntBox implements class IBox`1<int32> {",
            ".method public virtual instance int32 Get() { ldc.i4 1; ret }"]);
        Assert.AreEqual("end of class IntBox", closed.AddLine("}").Message);
        var wrong = Load([.. IBox, ".class public IntBox implements class IBox`1<int32> {",
            ".method public virtual instance string Get() { ldnull; ret }"]);
