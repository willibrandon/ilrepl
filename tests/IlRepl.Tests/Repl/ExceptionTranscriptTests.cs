...
/// <summary>
/// Real nested exceptions and observation limits remain understandable in both invocation and scenario reports.
/// </summary>
[TestClass]
public sealed class ExceptionTranscriptTests
{
    ...
    /// <param name="depth">Whether the revised exception chain exceeds the observation depth limit.</param>
    [TestMethod]
    [DataRow(false, false)]
    [DataRow(true, false)]
    [DataRow(false, true)]
    [DataRow(true, true)]
    public async Task ReportComparison_RendersNestedExceptionsAndProblems(bool caught, bool depth)
    {
        var session = IlLines.Load(ExceptionChainExamples.Method(false, false).Split('\n'));
        var edit = session.PrepareEdit("Read", "Copy");
        session.CommitEdit(edit.Name, ExceptionChainExamples.Method(false, true, depth));
        ...
        Assert.AreEqual(depth ? "incomplete" : "different", result.Outcome, result.Original.Detail + "; " + result.Edited.Detail);
        ...
        {
            Assert.AreEqual("completed", side.Outcome, side.Detail + "; " + side.StandardError);
            ...
            Assert.AreEqual(depth && side == result.Edited, last.Problem is not null);
