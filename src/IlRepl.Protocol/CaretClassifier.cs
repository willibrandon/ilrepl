...
/// <summary>
/// Says what the caret is on in a line of IL: which operand, which part of it, and what a
/// completion there would replace. It reads the line with the tokenizer's own grammar walkers over
/// a throwaway token list, so the palette and the colours agree on where a type or a member
/// begins and ends. The front-end and the host classify with the same code.
/// </summary>
public sealed class CaretClassifier
{
    ...
    public CompletionSite Classify(string line, int caret, bool inBlockComment)
    {
        ...
        var site = new Walk(_tokenizer, reader, line, caret).Line();
        return site is null || site.Kind == CompletionSiteKind.None ? CompletionSite.None with { Caret = caret } : site with { Caret = caret };
    }

    /// <summary>
    /// One classification: the walkers over a line's lexemes, each answering null when the caret
    /// is not in what it read.
    /// </summary>
    private sealed class Walk
    {
        private static readonly string[] Conventions = ["instance", "explicit", "vararg", "unmanaged", "cdecl", "stdcall", "thiscall", "fastcall", "default", "winapi", "platformapi"];
        ...
        public Walk(CilTokenizer tokenizer, CilLineReader reader, string line, int caret)
        {
            _tokenizer = tokenizer;
            _r = reader;
            _line = line;
            ...
            switch (kind)
            {
                ...
                case CilOperandKind.Switch:
                {
                    ...
                    for (var j = i + 1; j < (close < 0 ? _r.Count : close); j++)
                    {
                        ...
                        if (_r.IsName(j) && On(j))
                        {
                            return Site(CompletionSiteKind.Label, owner, _r.StartOf(j), _r.StartOf(j), _r.EndOf(j)) with { ArgumentIndex = index };
                        }
                    }
                ...
                {
                    var isArgument = owner.StartsWith("ldarg", StringComparison.Ordinal) || owner.StartsWith("starg", StringComparison.Ordinal);
                ...
                case CilOperandKind.Field:
                    ...
                    return Member(i, owner, kind == CilOperandKind.Field ? CompletionSiteKind.Field : owner == "newobj" ? CompletionSiteKind.Constructor : CompletionSiteKind.Method, complete: true);
                case CilOperandKind.Token:
                    if (_r.IsWord(i, "method") || _r.IsWord(i, "field"))
                    {
                        ...
                        return Member(i + 1, owner + (isField ? " field" : " method"), isField ? CompletionSiteKind.Field : CompletionSiteKind.Method, complete: true);
                    }
        ...
        private int SkipItemPrefix(int a)
        {
            ...
            while (_r.KindAt(a) == CilLexemeKind.AssemblyHint && (_r.TextAt(a).SequenceEqual("[in]") || _r.TextAt(a).SequenceEqual("[out]") || _r.TextAt(a).SequenceEqual("[opt]")))
            {
                a++;
            }
        ...
        private CompletionSite? MethodHeader(int i, string owner)
        {
            ...
            if (!hasType)
            {
                return _caret >= _r.EndOf(afterModifiers - 1) && (afterModifiers >= _r.Count || _caret <= _r.StartOf(afterModifiers)) ? Slot(CompletionSiteKind.Type, owner, -1, complete) : CompletionSite.None;
            }
        ...
        private CompletionSite? ClassHeader(int i, string owner)
        {
            ...
            if (_r.IsWord(j, "implements"))
            {
                ...
                while (a < _r.Count)
                {
                    ...
                    {
                        return (TypeAfter(a, end, "implements", complete) ?? CompletionSite.None) with { ArgumentIndex = index };
                    }
        ...
        /// <summary>
        /// The items of a parenthesised list, each a type with an optional slot, attribute, name,
        /// and initializer: a type site for the item the caret is in.
        /// </summary>
        private CompletionSite? Parameters(int open, int close, string owner, bool complete)
        {
            ...
            while (a < end)
            {
                ...
                if (typeEnd == start)
                {
                    ...
                    if (On(start))
                    {
                        return Site(CompletionSiteKind.Type, owner, _r.StartOf(start), _r.StartOf(start), _r.EndOf(start)) with { ArgumentIndex = index, DeclarationComplete = complete };
                    }
                ...
                if (_r.IsPunct(a, '='))
                {
                    ...
                    if (_caret >= _r.StartOf(a) && (literalEnd > a + 1 ? _caret <= _r.EndOf(literalEnd - 1) : true) && (a + 1 >= end || _caret <= _r.StartOf(a + 1) || literalEnd > a + 1))
                    {
                        return CompletionSite.None;
                    }
        ...
        /// <summary>
        /// A type expected at an index: the site inside it when the caret is there, an empty slot
        /// when the caret waits where the type would start, null when the caret is elsewhere.
        /// </summary>
        private CompletionSite? TypeAfter(int i, int end, string owner, bool complete)
        {
            if (end == i)
            {
                ...
                return _caret >= previousEnd && (i >= _r.Count || _caret <= _r.StartOf(i)) ? Slot(CompletionSiteKind.Type, owner, -1, complete) : null;
            }
        ...
        /// <summary>
        /// The site inside a type the caret is in: its name, a type argument, a modifier, or nothing
        /// when the caret is on a keyword or a suffix.
        /// </summary>
        private CompletionSite? Type(int i, int end, string owner, bool complete)
        {
            ...
            {
                while ((_r.IsWord(j, "native") || _r.IsWord(j, "unsigned")) && (_r.IsWord(j + 1, "native") || _r.IsWord(j + 1, "unsigned") || _r.IsPrimitive(j + 1)))
                {
                    j++;
                }
            ...
            if (nameEnd < 0)
            {
                ...
                return _caret >= (hint >= 0 ? _r.EndOf(hint) : _r.EndOf(j - 1)) ? Site(CompletionSiteKind.Type, owner, _caret, rangeStart, _caret) with { DeclarationComplete = complete } : CompletionSite.None;
            }
            ...
            if (onName || afterHint)
            {
                if (isParameter)
                {
                    return Site(CompletionSiteKind.GenericParameter, owner, _r.StartOf(nameStart), _r.StartOf(nameStart), _r.EndOf(nameEnd)) with { DeclarationComplete = complete };
                }
            ...
            while (a < end)
            {
                ...
                if (_caret <= _r.EndOf(argumentEnd - 1))
                {
                    ...
                    return site.Kind == CompletionSiteKind.Type
                        ? site with { Kind = CompletionSiteKind.TypeArgument, GenericOwnerText = ownerText, ArgumentIndex = index, SuppliedArguments = index }
                        : site;
                }
        ...
        /// <summary>
        /// A member reference: the calling convention, the return type, the declaring type, the
        /// <c>::</c>, the name, its generic arguments, and its parameters; or the session form.
        /// </summary>
        private CompletionSite? Member(int i, string owner, CompletionSiteKind memberKind, bool complete)
        {
            ...
            string? returnTypeText = declaringStart > j ? _line[_r.StartOf(j).._r.EndOf(declaringStart - 1)] : null;
            ...
            if (_caret <= _r.EndOf(separator - 1))
            {
                ...
                return site is { Kind: CompletionSiteKind.Type }
                    ? site with { Kind = CompletionSiteKind.MemberHead, ExplicitInstance = explicitInstance, ReturnTypeText = returnTypeText, NextIsDoubleColon = true }
            ...
            if (genericOpen >= 0 && genericClose >= 0 && paren < 0 && _caret >= _r.EndOf(genericClose) && (genericClose + 1 >= _r.Count || _caret <= _r.StartOf(genericClose + 1)))
            {
            ...
            if (paren >= 0 && Inside(paren, parenClose))
            {
                return (Parameters(paren, parenClose, owner, complete) ?? CompletionSite.None) with { DeclaringTypeText = declaringText, ReturnTypeText = returnTypeText, ExplicitInstance = explicitInstance };
            }
        ...
        /// <summary>
        /// A reference with no <c>::</c>: a session method or accessor in the form
        /// <c>[ret] Name[&lt;...&gt;](params)</c>, or the first type of a member reference still
        /// being typed. A bare name is a member head; an accessor's whole signature is one site.
        /// </summary>
        private CompletionSite? Session(int i, int j, int refEnd, string owner, bool complete, bool explicitInstance)
        {
            ...
            string? returnText = returnEnd > j ? _line[_r.StartOf(j).._r.EndOf(returnEnd - 1)] : null;
            ...
            if (bare < 0)
            {
                ...
                return head == returnEnd && _caret >= _r.EndOf(returnEnd - 1) && (returnEnd >= _r.Count || _caret <= _r.StartOf(returnEnd))
                    ? Slot(CompletionSiteKind.MemberHead, owner, -1, complete) with { ExplicitInstance = explicitInstance, ReturnTypeText = returnText }
                    : CompletionSite.None;
            }
            ...
            if (paren >= 0 && Inside(paren, _r.Matching(paren)))
            {
                return (Parameters(paren, _r.Matching(paren), owner, complete) ?? CompletionSite.None) with { ReturnTypeText = returnText, ExplicitInstance = explicitInstance };
            }
        ...
        /// <summary>
        /// The text of a type without its <c>class</c> or <c>valuetype</c> keyword: what a
        /// qualifier correction parses and re-spells.
        /// </summary>
        ...
        /// <summary>
        /// Whether a type read from <paramref name="from"/> to <paramref name="to"/> opens a bracket
        /// it never closes, so the caret after it is still inside the type.
        /// </summary>
        ...
        private CompletionSite Site(CompletionSiteKind kind, string owner, int prefixStart, int rangeStart, int rangeEnd)
        {
            ...
            return new CompletionSite(kind, owner, prefix, rangeStart, Math.Max(0, rangeEnd - rangeStart), _caret, null, null, false, -1, 0, null, false, false, false, true);
        }
