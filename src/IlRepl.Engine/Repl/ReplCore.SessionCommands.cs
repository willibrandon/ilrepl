...
public sealed partial class ReplCore
{
    private bool TrySessionAction(NormalizedLine line, out SessionAction? action)
    {
        ...
        try
        {
            return SessionCommand.TryParse(line.Text, ReferenceActions, out action);
        }
        catch (ArgumentException exception)
        {
            throw new ReplException(exception.Message, exception);
        }
    }
    ...
    private static string UnquotePath(string text)
    {
        try
        {
            return SessionCommand.UnquotePath(text);
        }
        catch (ArgumentException exception)
        {
            throw new ReplException(exception.Message, exception);
