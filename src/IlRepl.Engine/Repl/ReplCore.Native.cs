...
public sealed partial class ReplCore
{
    private HandleResult Native(string argument, bool diff = false)
    {
        ...
        if (diff)
        {
            ...
            {
                throw new ReplException("use .jit with --against for arbitrary native comparisons");
            }

            options = options with { Selector = edit.Name, Against = edit.Name, Original = true };
        }

        if (options.Assert && options.Against is null)
        {
            throw new ReplException("--assert requires a native comparison");
        }
    ...
    private NativeTarget CaptureNativeTarget(string selector, NativeOptions options)
    {
        ...
        if (selector.Length == 0 && !Session.Cell.IsEmpty)
        {
            return NativeCapture.Create(Session, "", options);
        }
        ...
        if (number is null)
        {
            throw new ReplException("there is no retained executable cell; use .jit cell <number>");
        }

        var boundary = Array.FindIndex(document.Entries, entry => entry.Number == number && entry.Kind == SessionEntryKind.Run);
        if (boundary < 0)
        {
            throw new ReplException($"cell {number} is not a retained executable cell");
        }
    ...
    internal void ReportNative(NativeReply reply)
    {
        ...
        if (reply.Right is { } right)
        {
            RenderNativeSide(right, !compared, addresses);
        }

        foreach (var line in reply.Difference)
        {
            NativeListing(NativeSymbolDisplay.Format(line, addresses));
        }
    }
    ...
    private void RenderNativeSide(NativeReport report, bool listing, NativeAddressFact[] addresses)
    {
        ...
        if (report.IsCapability)
        {
            RenderNativeInfo(report);
        }
        else if (report.Collectible)
        {
            Note("collectible static-access helpers; tiering unavailable");
        }
        ...
        foreach (var compilation in compilations)
        {
            ...
            {
                foreach (var header in NativeDisassembly.Headers(compilation, report.Raw))
                {
                    NativeListing("  " + header);
                }
            }
            ...
            if (listing)
            {
                ...
                foreach (var inlinee in compilation.Inlinees)
                {
                    Note("inlined: " + inlinee);
                }
            }
        }

        foreach (var problem in report.NormalizationProblems)
        {
        ...
        if (report.StandardOutput.Length != 0)
        {
            Listing("stdout: " + report.StandardOutput);
        }

        if (report.StandardError.Length != 0)
        {
            Listing("stderr: " + report.StandardError);
        }
    }
    ...
    private void RenderNativeInfo(NativeReport report)
    {
        ...
        Note("JIT: " + report.Jit);
        if (report.RuntimeIdentifier.Length != 0)
        {
            Note("platform: " + report.RuntimeIdentifier);
        }

        if (report.InstructionSets.Length != 0)
        {
            Note("effective ISA: " + NativeInstructionSets.Format(report.InstructionSets));
        }

        foreach (var (name, value) in report.Settings.OrderBy(pair => pair.Key, StringComparer.Ordinal))
        {
            Note(name + "=" + value);
        }
    }
