...
/// <summary>
/// Builds shared tokenizer and bootstrap vocabulary from authoritative opcode, command, directive, and keyword tables.
/// </summary>
public static class CilVocabularyBuilder
{
    ...
    private static CilVocabulary Build()
    {
        ...
        var directives = ReplDirectives.Names.Concat(ListingDirectives).Distinct(StringComparer.Ordinal)
        ...
        var commands = Completer.Commands.Select(c => c.Name).Concat(CommandAliases).Except(ReplDirectives.Names, StringComparer.Ordinal)
