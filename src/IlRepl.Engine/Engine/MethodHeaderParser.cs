...
/// For a session method the ILAsm words around the signature are accepted so a real header
/// pastes in, and ignored, because every session method is public and static. For a member of a
/// <c>.class</c> they mean what ILAsm says they mean.
/// </summary>
public static class MethodHeaderParser
{
    ...
    /// <summary>
    /// Parses the header of a method declared inside a <c>.class</c> block. The method is an
    /// instance method unless <c>static</c> is written, has no access unless an access word is
    /// written, and may be a constructor, a type initializer, or a generic method.
    /// </summary>
    ...
    public static MethodSignature ParseMember(string spec, ParseContext context, TypeHeader owner, out bool opensBlock, out bool closesBlock, out Type[] typeParameters, Func<string[], Type[]>? defineTypeParameters = null)
    {
        ...
        typeParameters = typeParameterNames.Length == 0 ? [] : defineTypeParameters?.Invoke(typeParameterNames) ?? PrototypeGenerics.Create(typeParameterNames);
        var memberContext = typeParameters.Length == 0 ? context : context.WithGenerics(context.Generics.WithMethodArguments(typeParameters));
        var returnStart = pos;
        var returnType = TypeParser.ParseAt(s, ref returnStart, memberContext, out _, out var returnRequired, out var returnOptional);
        ...
        var parameters = ParseParameters(s.Substring(firstParen + 1, close - firstParen - 1).Trim(), memberContext, allowVarArg: convention.HasFlag(CallingConventions.VarArgs));
        var constraints = typeParameterSpecs.Select((spec, i) => new GenericParameterDeclaration(spec.Name, spec.Attributes, [.. spec.ConstraintTexts.Select(t => TypeParser.Parse(t, memberContext))])).ToList();

        if (name == ".ctor")
        {
            if (isStatic || returnType != typeof(void))
            {
        ...
        {
            if (!isStatic || returnType != typeof(void) || parameters.Count > 0)
            {
    ...
    /// <summary>
    /// Finds the '(' that opens the parameter list: the first one that is not a modifier's
    /// argument, a function pointer's parameter list, or inside a generic parameter list.
    /// </summary>
    ...
    private static List<ArgumentDeclaration> ParseParameters(string inner, ParseContext context, bool allowVarArg)
    {
        ...
        foreach (var raw in TypeParser.SplitTopLevel(inner))
        {
            ...
            var at = 0;
            var type = TypeParser.ParseAt(part, ref at, context, out _, out var required, out var optional);
