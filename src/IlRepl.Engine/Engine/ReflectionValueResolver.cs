...
internal sealed partial class ReflectionValueResolver(
    MethodBase selected,
    IReadOnlyDictionary<MethodBase, MethodEditBody?> bodies,
    Func<ResolvedMethod, MethodBase> resolveMethod)
{
    ...
    {
        if (body.State.Entries[position].Instruction?.Operand is not ResolvedMethod resolved)
        {
            return null;
        }
    ...
    internal static Assembly? ActivationAssembly(string? name, Assembly context, bool fromFile)
    {
        ...
        try
        {
            if (!fromFile)
            {
                return LoadedAssembly(new AssemblyName(name), context);
            }
    ...
    private object?[]? Origin(MethodEditBody body, int position)
    {
        ...
        if (instruction?.ArgumentIndex is { } argument && instruction.Op.Name?.StartsWith("ldarg", StringComparison.Ordinal) == true
            && _parameters.TryGetValue((body.Method, argument), out var known))
        {
            return known;
        }

        if (_active.Count >= Limit || !_active.Add((body.Method, position)))
        {
            return null;
        }
    ...
    private object?[]? Collection(MethodEditBody body, int position, int fromTop)
    {
        ...
        if (stack is null || stack.Length < fromTop || stack[^fromTop].Origins is not { Length: 1 } origins)
        {
            return null;
        }

        var origin = origins[0];
        if (body.State.Entries[origin].Instruction?.Operand is not ResolvedMethod resolved)
        {
            return null;
        }
        ...
        if (method.DeclaringType != typeof(Type) && method.DeclaringType != typeof(TypeInfo)
            ...
            && !(typeof(MethodBase).IsAssignableFrom(method.DeclaringType!)
                && method.Name is nameof(MethodBase.GetParameters) or nameof(MethodBase.GetGenericArguments)))
        {
            return null;
        }
        ...
        for (var index = origin + 1; index < position; index++)
        {
            ...
            if (instruction is not null && instruction.Op != OpCodes.Nop
                && instruction.Op.Name?.StartsWith("ldc.i4", StringComparison.Ordinal) != true)
            {
                return null;
            }
        }
    ...
    private object?[]? Local(MethodEditBody body, int local)
    {
        ...
        if (stores.Any(entry => entry.Instruction!.Op.Name?.StartsWith("ldloca", StringComparison.Ordinal) == true
            && !ReadsMethodHandle(body, entry.index)))
        {
            return null;
        }
    ...
    {
        if (body.Method == selected)
        {
            return null;
        }

        var key = (body.Method, argument);
        if (_parameters.TryGetValue(key, out var known))
        {
            return known;
        }
        ...
        if (instructions.Any(entry => entry.Instruction?.ArgumentIndex == argument
            && entry.Instruction.Op.Name?.StartsWith("ldarga", StringComparison.Ordinal) == true))
        {
            return null;
        }
        ...
        foreach (var caller in bodies.Values.OfType<MethodEditBody>())
        {
            ...
            {
                if (caller.State.Entries[position].Instruction is not { Operand: ResolvedMethod resolved } instruction
                    || instruction.Op != OpCodes.Call && instruction.Op != OpCodes.Callvirt)
                {
                    continue;
                }
    ...
    private object?[]? Call(MethodEditBody body, int position, MethodBase method)
    {
        ...
        if (owner == typeof(IntrospectionExtensions) && name == nameof(IntrospectionExtensions.GetTypeInfo))
        {
            return Input(0);
        }

        if (owner == typeof(TypeInfo) && name == nameof(TypeInfo.AsType))
        {
            return Input();
        }

        if (name == "get_MethodHandle" && owner is not null && typeof(MethodBase).IsAssignableFrom(owner))
        {
            return Input();
        }
        ...
        {
            return [body.Method.Module.Assembly];
        }
        ...
        if (name == "get_Assembly" && owner == typeof(Type))
        {
            return Map(Input(), value => (value as Type)?.Assembly);
        }
        ...
        if (name is "get_Name" or "get_FullName" or "get_AssemblyQualifiedName")
        {
            ...
            if (owner == typeof(Assembly))
            {
                return Map(Input(), value => (value as Assembly)?.FullName);
            }
        }
        ...
        if (owner == typeof(Type) && method.IsStatic && name == nameof(Type.GetType))
        {
            if (method.GetParameters().Length >= 3 && method.GetParameters()[1].ParameterType != typeof(bool)
                && (Input(1) is not { } assemblies || assemblies.Any(value => value is not null)
                    || Input(2) is not { } types || types.Any(value => value is not null)))
            {
                return null;
            }
        ...
        {
            if (HasCustomBinder(body, position, method))
            {
                return null;
            }
            ...
            if (name == "get_TypeInitializer")
            {
                return Members(receiver, [ConstructorInfo.TypeConstructorName], MemberTypes.Constructor);
            }

            if (name == nameof(Type.GetEvent))
            {
                return Members(receiver, Input(argument), MemberTypes.Event);
            }
        ...
        if (owner == typeof(Delegate) && name == nameof(Delegate.CreateDelegate))
        {
            return DelegateTargets(body, position, method);
        }

        if (owner == typeof(Delegate) && name == "get_Method")
        {
            return Input();
        }
        ...
        if (!bodies.TryGetValue(definition, out var called) || called is null)
        {
            return null;
        }
    ...
    private bool HasCustomBinder(MethodEditBody body, int position, MethodBase method)
    {
        ...
        for (var index = 0; index < parameters.Length; index++)
        {
            if (parameters[index].ParameterType == typeof(Binder)
                && (Argument(body, position, index) is not { } values || values.Any(value => value is not null)))
            {
                return true;
            }
        }
    ...
    internal object?[]? DelegateTargets(MethodEditBody body, int position, MethodBase method)
    {
        ...
        if (member >= 0)
        {
            return Argument(body, position, member);
        }
    ...
    {
        if (receivers is null || names is null)
        {
            return null;
        }
    ...
    {
        if (left is null || right is null || left.Length * right.Length > Limit)
        {
            return null;
        }
