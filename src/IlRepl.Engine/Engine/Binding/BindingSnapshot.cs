...
public sealed class BindingSnapshot : IDisposable
{
    ...
    public static BindingSnapshot Capture(ParseContext context, IEnumerable<Assembly>? sessionAssemblies = null)
    {
            ...
            coreLibAssembly), sessionInstances, table)
        {
            ...
            Arguments = [.. context.Arguments.Select(argument => new VariableSymbol(RuntimeSymbolImporter.Import(argument.Type),
                argument.Name, false) { ExactType = argument.ExactType })],
