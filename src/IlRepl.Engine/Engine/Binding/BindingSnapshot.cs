...
public sealed class BindingSnapshot : IDisposable
{
    ...
    public static BindingSnapshot Capture(ParseContext context, IEnumerable<Assembly>? sessionAssemblies = null)
    {
            ...
            coreLibAssembly), sessionInstances, table)
        {
            ...
            Locals = [.. context.Locals.Select(l => new VariableSymbol(l.ExactType ?? RuntimeSymbolImporter.Import(l.Type), l.Name,
                l.IsPinned))],
            Arguments = [.. context.Arguments.Select(a => new VariableSymbol(a.ExactType ?? RuntimeSymbolImporter.Import(a.Type),
                a.Name, false))],
