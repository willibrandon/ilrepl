...
/// <summary>
/// Maps CIL primitive keywords and accepted C# aliases to their canonical symbols.
/// </summary>
/// <remarks>
...
public static class CilPrimitives
{
    private static readonly Dictionary<string, string> Canonical = new(StringComparer.Ordinal)
    {
        ...
        ["native unsigned int"] = "native uint",
        ["typedref"] = "typedref",
    ...
    private static readonly Dictionary<string, string> KeywordsByCoreLibName = CoreLibNames.ToDictionary(p => p.Value, p => p.Key,
    ...
    private static readonly Dictionary<string, Type> Types = new(StringComparer.Ordinal)
    {
        ...
        ["native int"] = typeof(nint),
        ["native uint"] = typeof(nuint),
        ["typedref"] = typeof(TypedReference),
    };

    /// <summary>
    /// Names the accepted C# alias for <c>System.Decimal</c>, which is not a CIL primitive.
    /// </summary>
    /// <remarks>
    ...
    public static Type TypeOf(string keyword)
    {
        ...
        return Types.TryGetValue(keyword, out var type) ? type : throw new ArgumentException($"'{keyword}' is not a primitive keyword",
            nameof(keyword));
    }
    ...
    {
        ArgumentNullException.ThrowIfNull(keyword);
        return CoreLibNames.TryGetValue(keyword, out var name) ? name : throw new ArgumentException(
            $"'{keyword}' is not a primitive keyword", nameof(keyword));
    }

    /// <summary>
    /// Returns the canonical keyword for a primitive CoreLib type, or null.
    /// </summary>
    /// <remarks>
