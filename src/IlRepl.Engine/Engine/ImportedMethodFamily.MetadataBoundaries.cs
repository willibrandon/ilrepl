...
internal sealed partial class ImportedMethodFamily
{
    ...
    private void ValidateMetadataBoundaries()
    {
        ...
        foreach (var body in _methods.Values.OfType<MethodEditBody>())
        {
            ...
            for (var position = 0; position < body.State.Entries.Count; position++)
            {
                ...
                if (_methods.ContainsKey(IlAsmRenderer.DefinitionOf(target)))
                {
                    continue;
                }
    ...
    private void ValidateIndirectMetadataBoundary(
        ReflectionValueResolver values,
        MethodEditBody body,
        int position,
    ...
    private void ValidateMetadataPayload(
        ReflectionValueResolver values,
        MethodEditBody body,
        int position,
        Instruction instruction,
        ...
        MethodBase external)
    {
        ...
        for (var argument = 0; argument < parameters.Length; argument++)
        {
            ...
            if (parameter != typeof(object) && parameter != typeof(object[]))
            {
                continue;
            }
    ...
    private bool IsCopiedMetadataMember(object value)
    {
        ...
        {
            return IsCopiedMetadataMember(parameter.Member);
        }
        ...
        if (value is Type type)
        {
            ...
            {
                return ContainsCopiedType(type);
            }
        ...
        if (value is not MemberInfo member)
        {
            return false;
        }
    ...
    private static bool IsMetadataIntrinsic(MethodBase method)
    {
        ...
        if (method.IsConstructor && typeof(Delegate).IsAssignableFrom(type))
        {
            return true;
        }
        ...
        if (IsIndirectReflection(method))
        {
            return false;
        }
