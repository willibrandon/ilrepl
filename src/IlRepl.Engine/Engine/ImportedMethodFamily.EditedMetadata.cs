...
internal sealed partial class ImportedMethodFamily
{
    private void ScanEditedMetadata(MethodEditBody body, Type owner)
    {
        ...
        foreach (var entry in body.State.Entries.Where(entry => entry.Custom is not null))
        {
            ...
            if (copied) AddMethod(attribute.Constructor);
            ...
            foreach (var (property, value) in attribute.NamedProperties)
            {
                ...
                if (copied)
                    foreach (var accessor in ImportedMetadata.Accessors(property))
                        if (_types.ContainsKey(DefinitionOf(accessor.DeclaringType!))) AddMethod(accessor);
            }
        }
        ...
        foreach (var mapping in body.State.Overrides)
        {
            ...
            for (var parent = owner.BaseType; parent is not null; parent = parent.BaseType) owners.Add(parent);
            ...
            if (target.IsFinal) throw new ReplException(MemberResolver.Describe(target) + " is final and cannot be overridden");
            ConsiderType(declaring, owner);
            ScanMethodSignature(target, null);
            var copied = _types.ContainsKey(DefinitionOf(declaring));
            if (copied) AddMethod(target, metadataOnly: true);
    ...
    private void ScanEditedAttributeValue(Type declared, object? value, Type owner, string location)
    {
        ...
        else if (value is Array array)
        {
            ...
            foreach (var item in array) ScanEditedAttributeValue(element, item, owner, location);
        }
    ...
    private void WriteOverrides(Type original, Dictionary<MemberInfo, IMemberDefinition> definitions, CecilWriter writer)
    {
        ...
        {
            if (!definitions.TryGetValue(IlAsmRenderer.DefinitionOf(body), out var method)) return;
        ...
        foreach (var (body, declaration) in ImportedMetadata.Overrides(original)) Add(body, declaration);
        ...
        foreach (var (method, body) in _methods.Where(pair => pair.Value is not null
            ...
            foreach (var declaration in body!.State.Overrides) Add(method, declaration.Target);
        foreach (var mapping in mappings.Values) mapping.Body.Overrides.Add(mapping.Target);
    }
}
