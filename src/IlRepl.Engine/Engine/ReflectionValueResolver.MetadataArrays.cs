...
internal sealed partial class ReflectionValueResolver
{
    ...
    private (HashSet<(MethodBase Method, int Origin)> Sites, bool Unknown) MetadataArrayAt(MethodEditBody body, int position, int fromTop)
    {
        ...
        if (_metadataArrayQueries.TryGetValue(key, out var known))
        {
            return known;
        }
    ...
    private bool CopiedArrayContents(
        MethodEditBody body,
        int position,
        int? fromTop,
        Func<object, bool> isCopiedMember,
        ...
        foreach (var candidate in bodies.Values.OfType<MethodEditBody>())
        {
            for (var index = 0; index < candidate.State.Entries.Count; index++)
            {
                ...
                if (instruction.Op.Name?.StartsWith("stelem", StringComparison.Ordinal) == true)
                {
                    ...
                    if (!array.Sites.Overlaps(stored.Sites) && !array.Unknown && !stored.Unknown)
                    {
                        continue;
                    }
                    ...
                    if (HasCopiedMetadata(candidate, index, 1, isCopiedMember))
                    {
                        return true;
                    }
                }
                else if (instruction.Op == OpCodes.Stind_Ref || instruction.Op == OpCodes.Stobj)
                {
                    ...
                    if (CopiedArrayAddress(candidate, index, array.Sites, active)
                        && HasCopiedMetadata(candidate, index, 1, isCopiedMember))
                    {
                        return true;
                    }
                }
    ...
    private bool CopiedArrayAddress(
        MethodEditBody body,
        int position,
        ...
        HashSet<(MethodBase Method, int Origin)> active)
    {
        ...
        if (!scalar.Unknown && scalar.Slots.Count != 0 && !scalar.Slots.OfType<(MethodEditBody Body, int Position)>().Any())
        {
            return false;
        }
    ...
    private bool CopiedArrayAddressOrigin(
        MethodEditBody body,
        int position,
        HashSet<(MethodBase Method, int Origin)> sites,
        HashSet<(MethodBase Method, int Origin)> active)
    {
        ...
        if (!active.Add((body.Method, position)))
        {
            return false;
        }
            ...
            if (instruction.LocalIndex is { } local && instruction.Op.Name?.StartsWith("ldloc", StringComparison.Ordinal) == true)
            {
                foreach (var entry in body.State.Entries.Select((entry, index) => (entry.Instruction, index)))
                {
                    ...
                    if (values is not null && values.Length != 0
                        && values[^1].Origins.Any(origin => CopiedArrayAddressOrigin(body, origin, sites, active)))
                    {
                        return true;
                    }
                }
