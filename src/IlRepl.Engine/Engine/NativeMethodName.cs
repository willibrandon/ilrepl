...
internal static class NativeMethodName
{
    ...
    internal static string[] Listings(MethodBase method)
    {
        ...
        {
            definition = ((MethodInfo)definition).GetGenericMethodDefinition();
        }
        ...
        string Build(bool canonical)
        {
            ...
            if (definition.IsGenericMethod)
            {
                name += "[" + string.Join(',', definition.GetGenericArguments().Select(t => TypeName(t))) + "]";
            }

            name += "(" + string.Join(',', definition.GetParameters().Select(parameter => TypeName(parameter.ParameterType))) + ")";
            if (definition is MethodInfo info && info.ReturnType != typeof(void))
            {
                name += ":" + TypeName(info.ReturnType);
            }
            ...
            string TypeName(Type type, bool alias = true)
            {
                ...
                if (alias && type.IsEnum)
                {
                    return TypeName(Enum.GetUnderlyingType(type));
                }

                if (alias && Aliases.TryGetValue(type, out var primitive))
                {
                    return primitive;
                }
                ...
                {
                    result += "[" + string.Join(',', type.GetGenericArguments().Select(t => TypeName(t))) + "]";
                }
