...
/// <summary>
/// Decodes one method body while preserving readable instructions when individual operands cannot resolve.
/// </summary>
internal sealed class MethodBodyReader
{
    ...
    /// <summary>
    /// Initializes the reader with the runtime method and its retained body image.
    /// </summary>
    public MethodBodyReader(MethodBase definition, MethodBase requested, Session session, MethodBodyImage body, List<string> notes)
    {
    ...
    /// <summary>
    /// Reads instructions, signatures and exception clauses into a recoverable listing.
    /// </summary>
    public DisassembledMethod Read()
    {
        ...
        return new DisassembledMethod(_definition, _requested, Header(), _body.MaxStack, _body.InitLocals, locals, context,
            entries, clauses, _notes, problems, _body.Il.Length, _body.Source);
    }
    ...
    private IReadOnlyList<IlSignature> ReadLocals()
    {
        ...
        {
            var locals = Try(() => MetadataSignatures.Locals(_metadata, _body.LocalSignatureToken, _provider, _generics),
                "the local signature");
        ...
        return reflected?.OrderBy(l => l.LocalIndex).Select(l => l.IsPinned ? IlSignature.Pinned(IlSignature.FromType(l
            .LocalType)) : IlSignature.FromType(l.LocalType)).ToList() ?? [];
    }
    ...
    private string Header()
    {
        ...
        var convention = (_definition.IsStatic ? "" : "instance ") + (_definition.CallingConvention
            .HasFlag(CallingConventions.VarArgs) ? "vararg " : "");
        var name = _definition is ConstructorInfo ? (_definition.IsStatic ? ".cctor" : ".ctor") : IlAsmRenderer
            .MemberName(_definition.Name);
        var generics = "";
        if (_definition is MethodInfo { IsGenericMethodDefinition: true })
        {
            generics = "<" + string.Join(", ", _methodArguments.Select(p => IlAsmRenderer
                .GenericParameterIlAsm(new GenericParameterDeclaration(p.Name, p.GenericParameterAttributes, p
                    .GetGenericParameterConstraints())))) + ">";
        }

        var signature = _metadata is null ? null : Try(
            () => MetadataSignatures.MethodDefinition(_metadata, _definition.MetadataToken, _provider, _generics),
        ...
        else
        {
            returnType = _definition is MethodInfo mi
                ? IlSignatureRenderer.IlAsmNamed(IlSignature.FromType(mi.ReturnType, mi.ReturnParameter
                    .GetRequiredCustomModifiers(), mi.ReturnParameter.GetOptionalCustomModifiers()))
                : "void";
            for (var i = 0; i < parameters.Length; i++)
            {
                var p = parameters[i];
        ...
        return $".method {words}{convention}{returnType} {name}{generics}({string.Join(", ", parameterTexts)}) {IlAsmWords
            .Implementation(_definition.GetMethodImplementationFlags())}";
    }
    ...
    private DisassembledEntry TokenInstruction(RawInstruction raw, OpCode emit)
    {
        ...
        switch (member)
        {
            case Type type:
            {
                var signature = _metadata is null ? null : Try(() => MetadataSignatures.TypeOperand(_metadata, token,
            ...
            {
                var text = FieldText(token, field) ?? IlAsmRenderer.RenderInstruction(new Instruction
                {
            ...
            default:
            {
                ...
                if (_metadata is not null && kind is not null)
                {
                    text = kind switch
                    {
                        HandleKind.TypeDefinition or HandleKind.TypeReference or HandleKind.TypeSpecification => Try(() =>
                            MetadataSignatures.TypeOperand(_metadata, token, _provider, _generics), null) is { } sig
                                ? IlSignatureRenderer.IlAsm(sig) : null,
                        HandleKind.FieldDefinition => FieldText(token, null) is { } fieldText ? "field " + fieldText : null,
                        HandleKind.MethodDefinition or HandleKind.MethodSpecification => MethodOperand(token, null, raw
                            .Label).Text is { } methodText ? "method " + methodText : null,
                        _ => null,
                    };
                }
    ...
    {
        var token = raw.Operand.Token;
        var signature = _metadata is null ? null : Try(() => MetadataSignatures.StandaloneMethod(_metadata, token, _provider,
            _generics), $"the signature at {raw.Label}");
        if (signature is null)
        {
            return Raw(raw, $"{emit.Name} 0x{token:x8}", _metadata is null ? $"the signature at {raw
                .Label} needs the module's metadata, which this assembly does not expose" : null);
        }
    ...
    private static string TypeOperandText(Type type) =>
        type.IsGenericType || type.HasElementType || type.IsGenericParameter || TypeNameFormatter.IsFunctionPointer(type)
            ? TypeNameFormatter.IlAsm(type) : TypeNameFormatter.IlAsmDeclaring(type);

    /// <summary>
    /// Classifies a token's metadata row, recording a note when a damaged or unavailable row cannot be read.
    /// </summary>
    private HandleKind? TokenKind(int token, string label)
    {
        ...
        try
        {
            ...
            return _metadata.GetMemberReference((MemberReferenceHandle)handle).GetKind() == MemberReferenceKind.Method
                ? HandleKind.MethodDefinition : HandleKind.FieldDefinition;
        }
    ...
    /// <summary>
    /// Reads a method operand's metadata spelling and runtime binding, using bare names for session methods.
    /// </summary>
    private (ResolvedMethod? Resolved, string? Text, bool EffectUnknown) MethodOperand(int token, MethodBase? method, string label)
    {
        if (method is not null && SessionAssemblies.TryGetDefinition(method.Module.Assembly, out var owner) && owner
            .Kind is SessionAssemblyKind.Trampoline or SessionAssemblyKind.Methods)
        {
        ...
        if (_metadata is not null)
        {
            memberSignature = Try(() => MetadataSignatures.MethodOperand(_metadata, token, _provider, _generics,
                out instantiation), method is null ? $"the method at {label}" : null);
        ...
        {
            _notes.Add($"the vararg call at {label} names optional argument types this assembly's metadata cannot be read for; "
                + "the stack after it is unknown");
            effectUnknown = true;
        }
    ...
    private string? FieldText(int token, FieldInfo? field)
    {
        ...
        var signature = Try(() => MetadataSignatures.FieldOperand(_metadata, token, _provider, _generics), field is null
            ? $"the field at {IlReader.LabelFor(0)}" : null);
        if (signature is null)
        {
            return null;
        }
    ...
    private (string Declaring, string Name) MemberRow(int token, MemberInfo? resolved)
    {
        ...
        catch (Exception ex) when (ex is BadImageFormatException or ArgumentException or InvalidOperationException)
        {
            ...
            return (resolved?.DeclaringType is { } declaring ? TypeNameFormatter.IlAsmDeclaring(declaring) : "?",
                resolved is null ? "?" : IlAsmRenderer.MemberName(resolved.Name));
        }
    }
    ...
    private (string Declaring, string Name) MemberRowCore(int token, MemberInfo? resolved)
    {
        ...
        switch (handle.Kind)
        {
            ...
            case HandleKind.MemberReference:
            {
                ...
                {
                    HandleKind.TypeDefinition or HandleKind.TypeReference or HandleKind.TypeSpecification => Try(() =>
                        MetadataSignatures.TypeOperand(_metadata, MetadataTokens.GetToken(row.Parent), _provider, _generics), null),
                    HandleKind.MethodDefinition => _provider.GetTypeFromDefinition(_metadata, _metadata
            ...
            case HandleKind.MethodSpecification:
                return MemberRowCore(MetadataTokens.GetToken(_metadata!
                    .GetMethodSpecification((MethodSpecificationHandle)handle).Method), resolved);
            default:
                name = resolved?.Name ?? "?";
                break;
        }
        ...
        {
            return (declaringType.IsGenericTypeDefinition ? IlSignatureRenderer.Declaring(IlSignature.Named(declaringType))
                : TypeNameFormatter.IlAsmDeclaring(declaringType), memberName);
        }
    ...
    private static string FloatText(uint bits)
    {
        ...
        if (float.IsFinite(value) && float.TryParse(text, NumberStyles.Float, CultureInfo.InvariantCulture, out var back)
            && BitConverter.SingleToInt32Bits(back) == unchecked((int)bits))
        {
    ...
    private static string DoubleText(ulong bits)
    {
        ...
        var text = value.ToString("R", CultureInfo.InvariantCulture);
        if (double.IsFinite(value) && double.TryParse(text, NumberStyles.Float, CultureInfo.InvariantCulture, out var back)
            && BitConverter.DoubleToInt64Bits(back) == unchecked((long)bits))
        {
            return text.Contains('.') || text.Contains('E') ? text : text + ".0";
        }
