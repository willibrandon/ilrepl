...
internal static class NativeScenarioCapture
{
    ...
    internal static (NativeMethodIdentity Scenario, NativeAssembly[] Images) Create(Session session, MethodInfo scenario,
        MethodBase selected, NativeOptions options, Func<MethodBase, NativeMethodIdentity> capture)
    {
        ...
        if (edit?.Current?.CallableEntryPoint is { } callable) replacements[callable] = selected;
        ...
        try
        {
            ...
            {
                if (helpers.ContainsKey(helper)) continue;
                capture(helper);
                ...
                foreach (var instruction in body.Body.Instructions)
                {
                    ...
                    if (method is null || replacements.ContainsKey(method)) continue;
                    var nested = session.Methods.FirstOrDefault(item => item.Trampoline.Method == method || item.Version.Body == method);
                    if (nested is not null) pending.Enqueue(nested.Version.Body);
                }
            }
            foreach (var (helper, image) in helpers)
            {
                ...
                var body = (MethodDefinition)module.LookupToken(helper.MetadataToken);
                foreach (var variable in body.Body.Variables) variable.VariableType = MapType(variable.VariableType);
                foreach (var handler in body.Body.ExceptionHandlers)
                    if (handler.CatchType is not null) handler.CatchType = MapType(handler.CatchType);
                foreach (var instruction in body.Body.Instructions)
                {
                    if (instruction.Operand is TypeReference type) instruction.Operand = MapType(type);
                    else if (instruction.Operand is FieldReference field)
                    {
                        ...
                        if (mapped != original) instruction.Operand = module.ImportReference((FieldInfo)mapped);
                    }
                ...
                foreach (var assembly in module.AssemblyReferences.ToArray()) Grant(module, assembly.Name);
                ...
                {
                    if (type is ByReferenceType byRef) return new ByReferenceType(MapType(byRef.ElementType));
                    if (type is PointerType pointer) return new PointerType(MapType(pointer.ElementType));
                    if (type is ArrayType array)
                    {
                        var mapped = new ArrayType(MapType(array.ElementType), array.Rank);
                        for (var index = 0; index < array.Dimensions.Count; index++) mapped.Dimensions[index] = array.Dimensions[index];
                        return mapped;
                    }
                    ...
                    if (!options.Original || edit?.Current is not { } family) return type;
        ...
        {
            foreach (var helper in helpers.Values) helper.Module.Dispose();
        }
    }
    ...
    private static void Grant(ModuleDefinition module, string assembly)
    {
        ...
        if (template is null || module.Assembly.CustomAttributes.Any(attribute => attribute.AttributeType == template.AttributeType
            && attribute.ConstructorArguments.Count != 0 && Equals(attribute.ConstructorArguments[0].Value, assembly))) return;
