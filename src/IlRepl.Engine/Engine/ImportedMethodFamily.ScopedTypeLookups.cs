...
internal sealed partial class ImportedMethodFamily
{
        ...
        OpCode call, int index)
    {
        ...
        foreach (var parameter in wrapper.Parameters.Skip(1)) il.Emit(OpCodes.Ldarg, parameter);
        il.Emit(call, target);
        il.Emit(OpCodes.Ret);
        return wrapper;
    }
    ...
    private static void LoadConstrainedLookupReceiver(MethodDefinition method, CecilInstruction call, MethodReference target)
    {
        ...
        if (!prefixes.Any(prefix => prefix.OpCode.Code == Code.Constrained)) return;
        prefixes.Reverse();
        ...
        foreach (var argument in arguments) method.Body.Variables.Add(argument);
        foreach (var argument in arguments.Reverse()) instructions.Add(il.Create(OpCodes.Stloc, argument));
        ...
        foreach (var argument in arguments) instructions.Add(il.Create(OpCodes.Ldloc, argument));
        ...
        if (tail) prefixes[^1].OpCode = OpCodes.Tail;
    }
}
