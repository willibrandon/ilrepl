...
internal sealed partial class ReflectionValueResolver
{
    ...
    private bool ArrayElementMayReferenceMetadata(MethodEditBody body, int position, int fromTop = 2)
    {
        ...
        if (element is not null && !MayBeMetadata(element))
        {
            return false;
        }
    ...
    {
        if (_arrayElementsActive.Count >= Limit || !_arrayElementsActive.Add((body.Method, position)))
        {
            return null;
        }
            ...
            foreach (var candidate in bodies.Values.OfType<MethodEditBody>())
            {
                for (var index = 0; index < candidate.State.Entries.Count; index++)
                {
                    ...
                    if (instruction.Op.Name?.StartsWith("stelem", StringComparison.Ordinal) == true)
                    {
                        ...
                        if (!array.Sites.Overlaps(stored.Sites) && !stored.Unknown)
                        {
                            continue;
                        }
                        ...
                        if (indices is not null && destination is not null
                            ...
                            && !indices.Intersect(destination).Any())
                        {
                            continue;
                        }
                        ...
                        if (incoming is null)
                        {
                            ...
                            if (type is null || MayBeMetadata(type) || type.IsByRef)
                            {
                                return null;
                            }
                    ...
                    else if (instruction.Op == OpCodes.Ldelema && array.Sites.Overlaps(ArrayAt(candidate, index, 2).Sites))
                    {
                        return null;
                    }
                        ...
                        && (instruction.Op == OpCodes.Call || instruction.Op == OpCodes.Callvirt || instruction.Op == OpCodes.Newobj))
                    {
                        ...
                        if (called.DeclaringType == typeof(Array) && called.Name == nameof(Array.GetValue))
                        {
                            continue;
                        }
                        ...
                        {
                            if (array.Sites.Overlaps(ArrayAt(candidate, index, argument).Sites))
                            {
                                return null;
                            }
                        }
    ...
    private void AddArrayStack(
        ...
        int position,
        int fromTop,
        HashSet<(MethodBase Method, int Origin)> sites,
        ref bool unknown)
    {
        var stack = body.State.Analysis.Before[position]?.Values;
        ...
        {
            AddArrayOrigin(body, origin, sites, ref unknown);
        }
    }
    ...
    private void AddArrayOrigin(MethodEditBody body, int position, HashSet<(MethodBase Method, int Origin)> sites, ref bool unknown)
    {
        ...
        try
        {
            ...
            if (op == OpCodes.Ldnull || op == OpCodes.Ldstr || op == OpCodes.Box)
            {
                return;
            }
            ...
            if (instruction.LocalIndex is { } local && op.Name?.StartsWith("ldloc", StringComparison.Ordinal) == true)
            {
                ...
                for (var index = 0; index < body.State.Entries.Count; index++)
                {
                    ...
                    if (entry?.LocalIndex != local)
                    {
                        continue;
                    }

                    if (entry.Op.Name?.StartsWith("ldloca", StringComparison.Ordinal) == true)
                    {
                        unknown = true;
                    }

                    if (entry.Op.Name?.StartsWith("stloc", StringComparison.Ordinal) != true)
                    {
                        continue;
                    }
                ...
                && (op == OpCodes.Ldfld || op == OpCodes.Ldsfld || op == OpCodes.Ldflda || op == OpCodes.Ldsflda))
            {
                ...
                foreach (var candidate in bodies.Values.OfType<MethodEditBody>())
                {
                    for (var index = 0; index < candidate.State.Entries.Count; index++)
                    {
                        ...
                        if (entry.Op != OpCodes.Stfld && entry.Op != OpCodes.Stsfld)
                        {
                            continue;
                        }
            ...
            {
                var called = resolveMethod(resolved);
                if (op == OpCodes.Newobj && called.DeclaringType?.IsArray != true)
                {
                    return;
                }
                ...
                if (bodies.TryGetValue(IlAsmRenderer.DefinitionOf(called), out var helper) && helper is not null)
                {
                    ...
                    {
                        if (helper.State.Entries[index].Instruction?.Op != OpCodes.Ret)
                        {
                            continue;
                        }
                ...
                if (called is MethodInfo info && !info.ReturnType.IsArray && info.ReturnType != typeof(object)
                    && !info.ReturnType.IsInterface && info.ReturnType != typeof(Array))
                {
                    return;
                }
            }
    ...
    private void AddArrayParameter(MethodEditBody body, int argument, HashSet<(MethodBase Method, int Origin)> sites, ref bool unknown)
    {
        ...
        for (var index = 0; index < body.State.Entries.Count; index++)
        {
            ...
            if (instruction?.ArgumentIndex != argument)
            {
                continue;
            }

            if (instruction.Op.Name?.StartsWith("ldarga", StringComparison.Ordinal) == true)
            {
                unknown = true;
            }

            if (instruction.Op.Name?.StartsWith("starg", StringComparison.Ordinal) != true)
            {
                continue;
            }
        ...
        foreach (var caller in bodies.Values.OfType<MethodEditBody>())
        {
            for (var position = 0; position < caller.State.Entries.Count; position++)
            {
                ...
                var called = resolveMethod(resolved);
                if (instruction.Op == OpCodes.Newobj && parameter < 0 || IlAsmRenderer.DefinitionOf(called) != body.Method)
                {
                    continue;
                }
