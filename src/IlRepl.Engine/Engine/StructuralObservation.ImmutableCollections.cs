...
internal sealed partial class StructuralObservation
{
    private ObservedValue? CaptureImmutableCollection(object value, int depth, int identity)
    {
        ...
        if (!dictionary && definition != typeof(ImmutableHashSet<>) && definition != typeof(ImmutableHashSet<>.Builder)) return null;
        var name = TypeName(type);
        var members = new List<ObservedMember>
        {
            new("comparer", Capture(type.GetProperty("KeyComparer")!.GetValue(value), depth + 1)),
        };
        if (dictionary) members.Add(new ObservedMember("value comparer", Capture(type.GetProperty("ValueComparer")!.GetValue(value),
            depth + 1)));
        ...
        try
        {
            ...
            {
                if (entries.Count >= MaximumNodes - _nodes) return Incomplete("collection exceeds the observation limit");
                if (dictionary)
                {
                    var entry = ((IDictionaryEnumerator)iterator).Entry;
                    entries.Add(("", entry.Key, entry.Value));
                }
                else entries.Add(("", iterator.Current, null));
            }
        }
        ...
        foreach (var (entry, index) in entries.Select((entry, index) => (entry, index)))
        {
            ...
            else item = Capture(entry.Key, depth + 1);
    ...
    private string? CollectionOrder(object? key, int depth)
    {
        ...
        if (!AppendOrder(observation, text, 1_048_576 - owner._orderingCharacters)) return null;
    ...
    private static bool AppendOrder(ObservedValue value, StringBuilder text, int limit)
    {
        ...
        if (!Append(value.Kind) || !Append(value.Type) || !Append(value.Value)
            ...
            || !Append(value.Members.Count.ToString(CultureInfo.InvariantCulture))) return false;
        foreach (var member in value.Members)
        {
            if (!Append(member.Name) || !AppendOrder(member.Value, text, limit)) return false;
        }
        ...
        {
            if (text.Length + (part?.Length ?? 0) + 12 > limit) return false;
