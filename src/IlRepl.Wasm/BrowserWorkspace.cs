...
public static partial class BrowserWorkspace
{
    ...
    private static SessionDocument? s_download;
    ...
    public static void Attach(PromptState prompt)
    {
        ...
        {
            if (input is not Hex1bKeyEvent { Key: Hex1bKey.F12 } key
                || key.Modifiers != (Hex1bModifiers.Control | Hex1bModifiers.Alt | Hex1bModifiers.Shift))
            {
                return false;
            }
    ...
    public static async Task<string> ExecuteAsync(string operation, string value)
    {
        ...
        if (operation == "saved")
        {
            await controller.SessionAsync(new SessionRequest
            {
                Action = new SessionAction { Operation = SessionOperation.AcknowledgeSave, Path = value },
                Document = s_download ?? throw new InvalidOperationException("there is no pending download"), Editor = controller.Editor,
            }, CancellationToken.None).ConfigureAwait(false);
            s_download = null;
            return "";
        }
